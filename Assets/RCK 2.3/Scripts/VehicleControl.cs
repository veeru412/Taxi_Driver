...
public class VehicleControl : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (activeControl)
        {
            ...
            else if (controlMode == ControlMode.touch)
            {
                ...
                if (accelFwd != 0) { inPut.accel = accelFwd; } else { inPut.accel = accelBack; }
                inPut.steer = Mathf.MoveTowards(inPut.steer, steerAmount, 0.07f);
        ...
        if (carSetting.carSteer)
            carSetting.carSteer.localEulerAngles = new Vector3(steerCurAngle.x, steerCurAngle.y, steerCurAngle.z + (inPut.steer * -120.0f));



        if (carSetting.automaticGear && (currentGear == 1) && (inPut.accel < 0.0f))
        {
            if (speed < 5.0f)
                ShiftDown();
        ...
        else if (carSetting.automaticGear && (currentGear == 0) && (inPut.accel > 0.0f))
        {
            if (speed < 5.0f)
                ShiftUp();

        }
        else if (carSetting.automaticGear && (motorRPM > carSetting.shiftUpRPM) && (inPut.accel > 0.0f) && speed > 10.0f && !inPut.brake)
        {
      ...
      //  carSetting.shiftCentre.x = -Mathf.Clamp(steer * (speed / 100), -0.03f, 0.03f);
        ...
        {
            if (inPut.brake || inPut.accel < 0 || speed < 1.0f)
            {
        ...
        wantedRPM = (5500.0f * inPut.accel) * 0.1f + wantedRPM * 0.9f;
        ...
        foreach (WheelComponent w in wheels)
        {
            ...
            if (w.drive)
            {
                if (!NeutralGear && inPut.brake && currentGear < 2)
                {
                    rpm += inPut.accel * carSetting.idleRPM;
                ...
                else
                {
                    if (!NeutralGear)
                    {
                        ...
                        rpm += (carSetting.idleRPM* inPut.accel);
                    }
                }
            ...
            if (inPut.brake || inPut.accel < 0.0f)
            {

                if ((inPut.accel < 0.0f) || (inPut.brake && (w == wheels[2] || w == wheels[3])))
                {

                    if (inPut.brake && (inPut.accel > 0.0f))
                    {
                        slip = Mathf.Lerp(slip, 5.0f, inPut.accel * 0.01f);
                    }
                    else if (speed > 1.0f)
                    {
            ...
            else
            {
                ...
                col.brakeTorque = inPut.accel == 0 || NeutralGear ? col.brakeTorque = 1000 : col.brakeTorque = 0;


                slip = speed > 0.0f ?
    (speed > 100 ? slip = Mathf.Lerp(slip, 1.0f + Mathf.Abs(inPut.steer), 0.02f) : slip = Mathf.Lerp(slip, 1.5f, 0.02f))
    : slip = Mathf.Lerp(slip, 0.01f, 0.02f);
            ...
            fc.extremumSlip = 0.2f + Mathf.Abs(inPut.steer);
            ...
            if (inPut.shift && (currentGear > 1 && speed > 50.0f) && inPut.shifmotor && Mathf.Abs(inPut.steer) < 0.2f)
            {

                if (powerShift == 0) { inPut.shifmotor = false; }
                ...
                if (carParticles.brakeParticlePerfab)
                {
                    ...
                    for (int i = 0; i < carSetting.hitGround.Length; i++)
                    {
                        ...
                        if (hit.collider.CompareTag(carSetting.hitGround[i].tag))
                        {
                            ...
                            if ((inPut.brake || Mathf.Abs(hit.sidewaysSlip) > 0.5f) && speed > 1)
                            {
                    ...
                    if (WGrounded && speed > 5 && !inPut.brake)
                    {
                    ...
                    else if ((inPut.brake || Mathf.Abs(hit.sidewaysSlip) > 0.6f) && speed > 1)
                    {

                        if ((inPut.accel < 0.0f) || ((inPut.brake || Mathf.Abs(hit.sidewaysSlip) > 0.6f) && (w == wheels[2] || w == wheels[3])))
                        {
        ...
        foreach (WheelComponent w in wheels)
        {
            ...
            if (w.drive)
            {
                ...
                else
                {
                    ...
                    if (!inPut.brake && inPut.accel != 0 && NeutralGear == false)
                    {
            ...
            if (inPut.brake || slip2 > 2.0f)
            {
                col.steerAngle = Mathf.Lerp(col.steerAngle, inPut.steer * w.maxSteer, 0.02f);
            }
            else
            {

                float SteerAngle = Mathf.Clamp(speed / carSetting.maxSteerAngle, 1.0f, carSetting.maxSteerAngle);
                col.steerAngle = inPut.steer * (w.maxSteer / SteerAngle);
        ...
        else
        {
            ...
            if ((Pitch > PitchDelay || inPut.accel > 0) && shiftTime == 0.0f)
            {
